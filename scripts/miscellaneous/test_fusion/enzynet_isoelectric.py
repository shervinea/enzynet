'Best architecture of 32x32x32 with isoelectric points information'
...
from enzynet import VolumeDataGenerator, MetricsHistory
...
from local_utils import Probabilities
...
validation_generator = \
    VolumeDataGenerator(v_size = v_size,
                        flips = (0, 0, 0), # No flip
                        batch_size = batch_size,
                        shuffle = False, # Validate with same set
                        p = p,
                        max_radius = max_radius,
                        noise_treatment = noise_treatment,
                        weights = weights,
...
if mode_run == 'test':
    ...
    p_val = Probabilities(weights = weights).predict(model, partition['validation'], dictionary)
    p_test = Probabilities(weights = weights).predict(model, partition['test'], dictionary)

    # Save results
    np.save('results/' + current_file_name + '_val.npy', p_val)
    np.save('results/' + current_file_name + '_test.npy', p_test)
